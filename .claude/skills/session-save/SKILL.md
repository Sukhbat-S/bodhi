...
2. **If there ARE changes to commit:**
   - Draft a concise commit message (imperative, 1-2 sentences)
   - Stage relevant files (never .env, .env.local, or credentials)
   - Commit with the message, always ending with:
...
- If the commit step fails, still proceed with memory extraction — saving knowledge is more important than the commit.
- If BODHI is unreachable, still complete the commit. Report "BODHI offline — session knowledge not saved" so it's visible.

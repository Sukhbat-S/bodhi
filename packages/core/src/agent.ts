...
export class Agent {
  ...
  async chat(
    ...
    const task = await this.backend.execute(fullPrompt, {
      model: this.config.model.includes("mythos") ? "mythos" : this.config.model.includes("opus") ? "opus" : "sonnet",

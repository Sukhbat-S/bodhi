...
function deployStateColor(state: string): string {
  switch (state.toUpperCase()) {
    ...
    default: return "text-steppe-smoke";
  }
}
...
function Skeleton({ className = "" }: { className?: string }) {
  return <div className={`skeleton bg-steppe-sky/30/40 rounded ${className}`} />;
}
...
function Section({ title, count, defaultOpen = true, children }: {
  ...
  return (
    <div className="rounded-xl border border-steppe-shadow/60 bg-steppe-sky/20/50 overflow-hidden">
      ...
      >
        <span className="text-xs uppercase tracking-wider text-steppe-smoke/70 flex items-center gap-2">
          {title}
          {count != null && count > 0 && (
            <span className="px-1.5 py-0.5 text-[10px] rounded-full bg-steppe-gold/15 text-amber-400 font-medium">{count}</span>
          )}
        </span>
        <svg className={`w-4 h-4 text-steppe-smoke/50 transition-transform ${open ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function ReflectionPage() {
  ...
  return (
    <div className="h-full flex flex-col">
        ...
        <div className="max-w-6xl mx-auto p-6 md:p-8 space-y-6 fade-in-up">
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1">
              <h1 className="text-3xl font-light text-steppe-cream/90">
                {greeting.split("**").map((part, i) =>
                  i % 2 === 1 ? <span key={i} className="font-medium">{part}</span> : part
                )}
              </h1>
              <p className="text-sm text-steppe-smoke/70 mt-1">{subtext}</p>
              {/* Day progress */}
              <div className="mt-3 flex items-center gap-2">
                <div className="flex-1 h-0.5 bg-steppe-sky/30/60 rounded-full overflow-hidden">
                ...
                <span className="text-[10px] text-steppe-smoke/50">{Math.round(dayProgress)}% of day</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
            ...
            <div className="md:col-span-3 space-y-6">
              ...
              <div className="rounded-xl border border-amber-500/20 bg-gradient-to-br from-steppe-sky/30 to-steppe-sky/15 p-6 shadow-[0_0_20px_rgba(217,119,6,0.06)]">
                {phase === "morning" && (
                  <div>
                    <p className="text-xs text-steppe-gold/70 uppercase tracking-wider mb-3">Morning Briefing</p>
                    {latestBriefing && isTodayBriefing(latestBriefing) ? (
                      ...
                      <div>
                        <p className="text-sm text-steppe-smoke/70 mb-2">Briefing generates at 8:00am</p>
                ...
                {phase === "afternoon" && (
                  <div>
                    <p className="text-xs text-steppe-gold/70 uppercase tracking-wider mb-3">Build Pulse</p>
                    <div className="space-y-3 mb-4">
                      {githubData?.commits[0] && (
                        <div className="flex items-center gap-2 text-sm">
                          <span className="text-steppe-smoke/50 font-mono text-xs">{githubData.commits[0].sha.slice(0, 7)}</span>
                          <span className="text-steppe-cream/80 truncate">{githubData.commits[0].message.split("\n")[0]}</span>
                          <span className="text-[10px] text-steppe-smoke/70">{timeAgo(githubData.commits[0].date)}</span>
                        </div>
                      )}
                      {deployments[0] && (
                        <div className="flex items-center gap-2 text-sm">
                          <span className={`text-xs font-medium ${deployStateColor(deployments[0].state)}`}>{deployments[0].state.toUpperCase()}</span>
                          <span className="text-steppe-smoke">{deployments[0].meta?.commitMessage?.split("\n")[0] || deployments[0].name}</span>
                        </div>
                      )}
                      {openPRs.length > 0 && (
                        <p className="text-sm text-steppe-smoke">{openPRs.length} open PR{openPRs.length > 1 ? "s" : ""} waiting for review</p>
                      )}
                    ...
                    <button
                      ...
                      className="px-4 py-2 text-sm font-medium text-amber-400 bg-steppe-gold/10 rounded-lg hover:bg-steppe-gold/15 transition-colors"
                    >
                      Focus mode &rarr;
                    </button>
                  </div>
                ...
                {phase === "evening" && (
                  <div>
                    <p className="text-xs text-steppe-gold/70 uppercase tracking-wider mb-3">Reflection</p>
                    {heroInsight && (
                      <div className="mb-4">
                        <p className="text-sm text-steppe-cream/80 leading-relaxed">{heroInsight.text}</p>
                        <button
                          onClick={() => navigate(getInsightAction(heroInsight).to)}
                          className="mt-2 text-xs text-steppe-gold/70 hover:text-amber-400 transition-colors"
                        >
                ...
                {phase === "night" && (
                  <div>
                    <p className="text-xs text-steppe-gold/70 uppercase tracking-wider mb-3">Wind Down</p>
                    <div className="grid grid-cols-3 gap-4 mb-4">
                      <div><p className="text-2xl font-semibold text-steppe-cream/90">{stats?.recentCount ?? 0}</p><p className="text-xs text-steppe-smoke/70 mt-1">this week</p></div>
                      <div><p className="text-2xl font-semibold text-steppe-cream/90">{goals.length}</p><p className="text-xs text-steppe-smoke/70 mt-1">active goals</p></div>
                      <div><p className="text-2xl font-semibold text-steppe-cream/90">{stats?.totalMemories ?? 0}</p><p className="text-xs text-steppe-smoke/70 mt-1">total memories</p></div>
                    </div>
                    <p className="text-sm text-steppe-smoke italic">{getDailyPrompt()}</p>
                  </div>
                )}
              ...
              <Section title="Active Work" count={activeSessions.length + (tasks.length || 0) + openPRs.length} defaultOpen={true}>
                ...
                {activeSessions.length > 0 && (
                  <div className="mb-3">
                    <p className="text-[10px] text-steppe-smoke/50 uppercase tracking-wider mb-2">Live Sessions</p>
                      ...
                      {activeSessions.map((s) => {
                        ...
                        return (
                          <div key={s.id} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-steppe-sky/30/50 border border-steppe-shadow/40">
                            ...
                            <span className="text-xs text-steppe-cream/90 font-medium">{s.id}</span>
                            <span className="text-[10px] text-steppe-smoke/70 truncate max-w-32">
                      ...
                      <div className="space-y-1">
                        {sessionMessages.slice(-3).map((m) => (
                          <div key={m.id} className="flex items-center gap-1.5 text-[11px]">
                            ...
                            <span className="text-steppe-smoke/30">{m.toSession ? `→ ${m.toSession}` : "•"}</span>
                            <span className="text-steppe-smoke/70 truncate">{m.message}</span>
                          </div>
                        ))}
                      </div>
                    )}
                ...
                {tasks.length > 0 && (
                  <div className="mb-3">
                    <p className="text-[10px] text-steppe-smoke/50 uppercase tracking-wider mb-2">Tasks</p>
                    <div className="space-y-1.5">
                      {tasks.slice(0, 5).map((t) => (
                        <div key={t.id} className="flex items-center gap-2 text-sm">
                          <span className="w-1.5 h-1.5 rounded-full bg-violet-400 shrink-0" />
                          <span className="text-steppe-cream/80 truncate flex-1">{t.title}</span>
                          {t.status && <span className="text-[10px] text-steppe-smoke/50">{t.status}</span>}
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {openPRs.length > 0 && (
                  <div className="mb-3">
                    <p className="text-[10px] text-steppe-smoke/50 uppercase tracking-wider mb-2">Open PRs</p>
                    <div className="space-y-1.5">
                      {openPRs.slice(0, 3).map((pr) => (
                        <div key={pr.number} className="flex items-center gap-2 text-sm">
                          ...
                          <span className="text-steppe-cream/80 truncate flex-1">{pr.title}</span>
                          <span className="text-[10px] text-steppe-smoke/50">{pr.repo.split("/")[1]}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {tasks.length === 0 && openPRs.length === 0 && activeSessions.length === 0 && (
                  <p className="text-sm text-steppe-smoke/70">All clear.</p>
                )}
              </Section>
              ...
              <Section title="Communications" count={(unreadCount || 0) + conversations.length} defaultOpen={!!unreadCount && unreadCount > 0}>
                ...
                {emails.length > 0 && (
                  <div className="mb-3">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-[10px] text-steppe-smoke/50 uppercase tracking-wider">Email</p>
                      <button onClick={() => navigate("/inbox")} className="text-[10px] text-steppe-gold/70 hover:text-amber-400">inbox &rarr;</button>
                    </div>
                    <div className="space-y-1.5">
                      {emails.slice(0, 4).map((e) => (
                        <div key={e.id} className="flex items-start gap-2 text-sm">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className={`truncate ${e.isUnread ? "text-steppe-cream/90 font-medium" : "text-steppe-smoke"}`}>{e.subject}</p>
                            <p className="text-[10px] text-steppe-smoke/50">{e.from.split("<")[0].trim()} &middot; {timeAgo(e.date)}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {conversations.length > 0 && (
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-[10px] text-steppe-smoke/50 uppercase tracking-wider">Recent Conversations</p>
                      <button onClick={() => navigate("/chat")} className="text-[10px] text-steppe-gold/70 hover:text-amber-400">chat &rarr;</button>
                    </div>
                    <div className="space-y-1.5">
                      {conversations.map((c) => (
                        <div
                          ...
                          className="flex items-center gap-2 text-sm cursor-pointer hover:bg-steppe-sky/30/30 rounded-lg p-1 -mx-1 transition-colors"
                        >
                          <span className="w-1.5 h-1.5 rounded-full bg-steppe-smoke/40 shrink-0" />
                          <span className="text-steppe-smoke truncate flex-1">{c.title || "Untitled"}</span>
                          <span className="text-[10px] text-steppe-smoke/50 shrink-0">{c.channel}</span>
                          <span className="text-[10px] text-steppe-smoke/50 shrink-0">{timeAgo(c.lastActiveAt)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {emails.length === 0 && conversations.length === 0 && (
                  <p className="text-sm text-steppe-smoke/70">All quiet.</p>
                )}
              </Section>
              ...
              >
                ...
                {deployments.length > 0 && (
                  <div className="mb-3">
                    <p className="text-[10px] text-steppe-smoke/50 uppercase tracking-wider mb-2">Deployments</p>
                    <div className="space-y-1.5">
                      {deployments.slice(0, 3).map((d) => (
                        <div key={d.id} className="flex items-center gap-3 text-sm">
                          ...
                          <span className="text-steppe-smoke truncate flex-1">{d.meta?.commitMessage?.split("\n")[0] || d.name}</span>
                          <span className="text-[10px] text-steppe-smoke/50 shrink-0">{timeAgo(d.createdAt)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {githubData?.commits && githubData.commits.length > 0 && (
                  <div>
                    <p className="text-[10px] text-steppe-smoke/50 uppercase tracking-wider mb-2">Recent Commits</p>
                    <div className="space-y-1.5">
                      {githubData.commits.slice(0, 5).map((c) => (
                        <div key={c.sha} className="flex items-center gap-2 text-sm">
                          <span className="text-steppe-smoke/50 font-mono text-xs shrink-0">{c.sha.slice(0, 7)}</span>
                          <span className="text-steppe-smoke truncate flex-1">{c.message.split("\n")[0]}</span>
                          <span className="text-[10px] text-steppe-smoke/50 shrink-0">{timeAgo(c.date)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <p className="text-[10px] uppercase tracking-wider text-steppe-smoke/70 mb-1">{insight.type}</p>
                              <p className="text-sm text-steppe-cream/80 leading-relaxed line-clamp-2">{insight.text}</p>
                            </div>
                          </div>
                        </div>
                      );
            ...
            <div className="md:col-span-2 space-y-6">
                  ...
                  <div className="space-y-2">
                    {events.map((e) => (
                      <div key={e.id} className="flex items-center gap-3 text-sm">
                        ...
                        <span className="text-steppe-cream/80 truncate">{e.summary}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-2">
                    {goals.slice(0, 5).map((g) => (
                      <div key={g.id} className="flex items-start gap-2">
                        ...
                        <p className="text-sm text-steppe-cream/80 leading-relaxed line-clamp-1">{g.content}</p>
                      </div>
                    ))}
                    {goals.length > 5 && <p className="text-xs text-steppe-smoke/70">+{goals.length - 5} more</p>}
                  </div>
                </div>
              )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-steppe-sky/30/40 border border-steppe-shadow/30 text-xs text-steppe-smoke hover:border-amber-500/30 hover:text-steppe-cream/90 transition-colors"
                    >
                      <svg className="w-3.5 h-3.5 text-steppe-gold/60 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {latestBriefing && !(phase === "morning" && isTodayBriefing(latestBriefing)) && (
                <div className="rounded-xl border border-steppe-shadow/60 bg-steppe-sky/20/50 p-5">
                  ...
                  <div className="prose prose-invert prose-sm max-w-none prose-p:my-1 prose-headings:mt-2 prose-headings:mb-1 text-steppe-smoke line-clamp-6">
                    <ReactMarkdown remarkPlugins={[remarkGfm]}>
                      {latestBriefing.content}
                    </ReactMarkdown>
                  </div>
                  <button onClick={() => navigate("/briefings")} className="mt-3 text-xs text-steppe-gold/70 hover:text-amber-400 transition-colors">
                    All briefings &rarr;
                  </button>
                </div>
              )}
      ...
      {/* ─── Sticky Quick Input ─── */}
      <div className="sticky bottom-0 bg-gradient-to-t from-[var(--steppe-night)] via-[var(--steppe-night)]/95 to-transparent pt-6 pb-4 px-6 md:px-8">
        <form onSubmit={handleQuickSubmit} className="max-w-6xl mx-auto">
          <div className="flex items-center gap-2 mb-2">
            <button
              type="button"
              onClick={() => setQuickMode("chat")}
              className={`text-xs px-3 py-1 rounded-md transition-colors ${quickMode === "chat" ? "bg-steppe-gold/15 text-amber-400" : "text-steppe-smoke/70 hover:text-steppe-smoke"}`}
            >
              Chat
            </button>
            <button
              ...
              className={`text-xs px-3 py-1 rounded-md transition-colors ${quickMode === "remember" ? "bg-steppe-gold/15 text-amber-400" : "text-steppe-smoke/70 hover:text-steppe-smoke"}`}
            >
              Remember
            </button>
            {remembered && <span className="text-xs text-emerald-400 ml-2">Remembered</span>}
            <span className="text-[10px] text-steppe-smoke/30 ml-auto">press / to focus</span>
          </div>

...
describe("resolveProject", () => {
  it("resolves registered project by name", () => {
    ...
    const project = resolveProject("sample-project");
  ...
  it("resolves by path prefix", () => {
    addProject("path-project", {
      name: "PathProject",
      path: "/tmp/path-project",
      allowedTools: ["Read"],
      maxBudgetUsd: 5,
    });
    const project = resolveProject("/tmp/path-project/src/app");
    expect(project).not.toBeNull();
  });

...
{"digest": "...", "stats": {"commits": ${commits.length}, "memories": ${memories.length}}, "tweets": ["tweet-sized summary"]}`;
  ...
  // Sequential pipeline with memory guards. Each job waits for the previous
  // to finish and checks heap before starting. No parallel Puppeteer/Bridge.
  const tz = config.TIMEZONE || "Asia/Ulaanbaatar";
  let overnightRunning = false;

  const logMemory = (label: string) => {
    const mb = Math.round(process.memoryUsage().heapUsed / 1024 / 1024);
    console.log(`[overnight] ${label} — heap: ${mb}MB`);
  };

  const runOvernightPipeline = async () => {
    if (overnightRunning) {
      console.warn("[overnight] Previous pipeline still running — skipping");
      return;
    }
    overnightRunning = true;
    const pipelineStart = Date.now();
    console.log("[overnight] ─── Pipeline starting ───");
    logMemory("start");

    // Job 1: Design audit (Puppeteer screenshots)
    try {
      console.log("[overnight] [1/5] Design audit...");
      const audits = await designAuditor.auditAll();
      const avg = audits.reduce((s, a) => s + a.score, 0) / audits.length;
      console.log(`[overnight] [1/5] Design audit: avg ${avg.toFixed(1)}/10`);
      if (briefingStore) {
        const report = audits.map((a) => `**${a.page}** ${a.score}/10\n${a.issues.map((i) => `- ${i}`).join("\n")}`).join("\n\n");
        await briefingStore.save("morning", `## Nightly Design Audit\n\n${report}`).catch(() => {});
      }
    } catch (err) {
      console.error("[overnight] [1/5] Design audit failed:", err instanceof Error ? err.message : err);
    }
    logMemory("after design audit");

    // Job 2: Carousel generation (Puppeteer rendering)
    try {
      console.log("[overnight] [2/5] Carousel generation...");
      const result = await scheduler.trigger("content-generate");
      console.log(`[overnight] [2/5] Carousel: ${result.status}`);
    } catch (err) {
      console.error("[overnight] [2/5] Carousel failed:", err instanceof Error ? err.message : err);
    }
    logMemory("after carousel");

    // Job 3: Second carousel
    try {
      console.log("[overnight] [3/5] Second carousel...");
      const result = await scheduler.trigger("content-generate");
      console.log(`[overnight] [3/5] Carousel: ${result.status}`);
    } catch (err) {
      console.error("[overnight] [3/5] Second carousel failed:", err instanceof Error ? err.message : err);
    }
    logMemory("after second carousel");

    // GC hint between Puppeteer jobs and Bridge jobs
    if (global.gc) global.gc();

    // Job 4: Engineering health (Bridge)
    try {
      console.log("[overnight] [4/5] Engineering health...");
      ...
      console.log(`[overnight] [4/5] Engineering: ${result.status}`);
      if (briefingStore && result.result) {
        await briefingStore.save("morning", `## Nightly Engineering Audit\n\n${result.result}`).catch(() => {});
      }
    } catch (err) {
      console.error("[overnight] [4/5] Engineering check failed:", err instanceof Error ? err.message : err);
    }
    logMemory("after engineering");

    // Job 5: Security scan (Bridge)
    try {
      ...
      console.error("[overnight] [5/5] Security scan failed:", err instanceof Error ? err.message : err);
    }

    const elapsed = ((Date.now() - pipelineStart) / 60000).toFixed(1);
    logMemory("pipeline complete");
    console.log(`[overnight] ─── Pipeline done (${elapsed}min) ───`);
    overnightRunning = false;
  };

  // Single cron at 2:00 AM triggers the whole pipeline sequentially
  cron.schedule("0 2 * * *", runOvernightPipeline, { timezone: tz });
  console.log("  Overnight: sequential pipeline at 2:00 AM (design → carousel ×2 → engineering → security)");

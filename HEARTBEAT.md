...
> **Timezone**: Configured via BODHI_TIMEZONE env var (default: UTC)
...
## Morning (8:00-8:30 AM local time, weekdays only)
...
## Evening (20:00-20:30 PM local time, weekdays only)
...
## Weekly (Monday 9:00-9:30 AM local time)
...
## Daily (12:00 Noon local time)

- [ ] **External project quality check** (optional): If `BODHI_PROJECT_DIR` is set, run quality metrics:
  ```bash
  cd $BODHI_PROJECT_DIR && bash scripts/quality-check.sh --json
  ```
